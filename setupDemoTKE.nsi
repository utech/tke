...
!define MUI_STARTMENUPAGE_DEFAULTFOLDER "ТКЕнерго менеджер (Демо)"
...
!define MUI_FINISHPAGE_RUN_TEXT "Запустити демо програму обліку житлових будинків"
...
; Shortcuts
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\ТКЕнерго менеджер-абонент (демо).lnk" "$INSTDIR\bin\tkeAbonentDemo.EXE"
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\ТКЕнерго менеджер-організації (демо).lnk" "$INSTDIR\bin\tkeOrganizaciiDemo.EXE"
  CreateShortCut "$DESKTOP\ТКЕнерго менеджер-абонент (демо).lnk" "$INSTDIR\bin\tkeAbonentDemo.EXE"
  CreateShortCut "$DESKTOP\ТКЕнерго менеджер-організації (демо).lnk" "$INSTDIR\bin\tkeOrganizaciiDemo.EXE"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SEC01} "Файли, необхідні для роботи демо програми "
...
Section Uninstall
  ...
  Delete "$SMPROGRAMS\$ICONS_GROUP\ТКЕнерго менеджер-абонент (демо).lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\ТКЕнерго менеджер-організації (демо).lnk"
  Delete "$SMPROGRAMS\$ICONS_GROUP\Позбутися програми.lnk"
  Delete "$DESKTOP\ТКЕнерго менеджер-абонент (демо).lnk"
  Delete "$DESKTOP\ТКЕнерго менеджер-організації (демо).lnk"
